...
def search_candidates(user_id: int, token: str = access_token, version: str = api_version) -> int:
    """
    Функция, которая по указанным критериям ищет подходящего кандидата для юзера.
    При этом проверяет, чтобы профиль у кандидата был не закрытым, и чтобы он не был в списке уже просмотренных.
    Возвращает id подходящего кандидата.
    """
